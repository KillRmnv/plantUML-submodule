/// @file PlantUmlPngGenerator.cpp
/// @brief Реализация генератора PNG-диаграмм из PlantUML и сохранения в SC-память.
...
#include "../utils/base64.h"   // base64_encode

#ifdef _WIN32
/// @brief Макрос-обёртка для потокобезопасного получения локального времени на Windows.
#define LOCALTIME(time_ptr, tm_ptr) localtime_s(tm_ptr, time_ptr)
#else
/// @brief Макрос-обёртка для потокобезопасного получения локального времени на Unix-подобных системах.
#define LOCALTIME(time_ptr, tm_ptr) localtime_r(time_ptr, tm_ptr)
#endif

void PlantUmlPngGenerator::deleteTempFiles(
...
/// @brief Генерирует уникальное имя файла для результата (на основе времени и случайного числа).
/// @return Строка вида diagram_YYYY-M-D_H-M-S_RANDOM.base64.
std::string generateUniquePngName()
{
  ...
  std::tm tm {};
  LOCALTIME(&time, &tm);  // Кроссплатформенная обёртка

  // Случайное число для обеспечения уникальности имени
...
PlantUmlPngGenerator::~PlantUmlPngGenerator() = default;

ScAddr PlantUmlPngGenerator::png_generator(
    const std::string & plantUML_code,
    ScAddr outputNode)
{
  // 1. Создание временного .puml-файла с исходным PlantUML-кодом.
  ...
  // Измените команду в вашем коде:
  std::string cmd = "java  -jar "
                    "problem-solver/cxx/plantUML-submodule/utils/plantuml-1.2025.4.jar "
                    "-tpng " + pumlFile;
  ...
  // 3. Чтение сгенерированного PNG в память.
  ...
  // 4. Кодирование PNG в Base64-строку.
  ...
  // 5. Подготовка SC-stream с Base64-данными и запись в sc-ссылку.
  size_t size = encoded_data.size();
  sc_char * raw_data = new sc_char[size];
  std::copy(encoded_data.begin(), encoded_data.end(), raw_data);

  // Создаём постоянную ссылку и записываем в неё содержимое.
  ...
  if (stream && stream->IsValid() && stream->HasFlag(SC_STREAM_FLAG_READ))
  {
    ...
    // Устанавливаем формат содержимого как PNG через nrel_format/format_png.
    ...
    // Внимание: raw_data передан в ScStream без передачи владения (false).
    // При необходимости можно освободить его после SetLinkContent, если данные скопированы внутрь sc-памяти.
    return outputNode;
  }
